...
function App() {
  ...
  const [notes, setNotes] = useLocalStorage('note', [
    {title: 'New note', content:'', categories: ["sexy", "notSexy"] , createdAt: `${moment().format("DD-MM-YYYY|hh:mm:ss")}`},
  ])
  const [shownNoteIndex, setShownNoteIndex] = useState(0)
  const [animation, setAnimation] = useState(false)
  const [expanded, setExpanded] = useState(false)
  const [filteredCategories, setFilteredCategories] = useState([notes.categories])
  ...
  function createNote() {
    ...
    const newNote =  {title: 'New note', content:'', categories: ["test"], createdAt: `${date}`}
      ...
      {(filteredNotes.map((note: any, index:number) => {  
          return (
            ...
            (<div key={index} className={`note ${expanded ? 'expanded' : 'collapsed'}`}>
              ...
              <textarea placeholder="click here to edit!" value={notes[index].content} onChange={(e) => handleContentChange(index, e)} />
